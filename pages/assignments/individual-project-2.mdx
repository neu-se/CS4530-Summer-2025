...
nav_order: 7
deadline: 2025-05-19T18:00-04:00
---
...
Start by accepting our [TODO invitation TODO]. It will create a GitHub repository for you which will include the starter code for this assignment. You will set up the assignment as before. Do not install any additional dependencies: **package.json must be unchanged**.

## Tasks

The code you start this assignment with is a viable solution to the first individual assignment, with the following changes:

 - A few cosmetic changes (like an SVG favicon). TODO link
 - Making `checkAuth` and `populateSafeUserInfo` into functions that return Promises, and adding appropriate `async` and `await` annotations to make this work with the rest of the program. TODO link
 - Moving user models (TODO link), forum models (TODO link), and game models (TODO link) to MongoDB by adding Mongoose models and using the database to populate models.
 
### Task 1: Sharing biographical details

This task has two parts:

 - Review the feedback on your conditions of satisfaction from individual project 1, and revise these to conditions of satisfaction that you **will** be accountable for in the third individual project; submit this on Gradescope.
 - **Meet with a member of the course staff before the deadline**, review your conditions of satisfaction, and talk through your work breakdown — the set of tasks necessary to accomplish your project. You **may be asked to repeat this step**, and the assignment is not complete

You will also be working on your team project plan this week, which has much more extensive documentation requirements. There are several reasons for this:
 ...
 3. You're expected to progress further in your group project --- most people won't get far past the "essential" conditions for this user story, and most group projects will complete "desired" conditions.
 4. The written work for the group project plan is there to keep multiple people on the same page. As a general rule, projects that coordinate more people need more documentation to keep everyone on the same page.

You are responsible for starting early and meeting with a member of the course staff during office hours! This part of the assignment will be graded separately for the purpose of late days. Unlike other assignments, can be submitted up to 4 days late with a penalty of 4 per day.

### Task 2: A stateless server

*Most* of the models that were stored in memory for the first individual assignment have been moved to an appropriate repository layer, held in a MongoDB database. (The default setup for this project behaves the same way as the first project did: starting the server generates a brand-new MongoDB database which is destroyed when you stop your server.)
...
The one part of the task left to you is the `Chat` and `Message` models. We recommend you take the following steps:
 ...
 - Replace these with appropriate new Mongoose models in `server/src/models/chat.model.ts` and `server/src/models/message.model.ts`. Study the previous diffs, especially the forum ones, for examples.
...
Modify both the client and the server to play Tic-Tac-Toe.
 ...
 - When the game is active:
   ...
   - The player should see text in each of the spaces where a move has been made, and an "Add X" or "Add O" in every open spot (X if they're the X player, O if they're the O player). These buttons should be visibly and actually disabled when it is not their turn.
...
The web server backend needs to change:
 ...
 - This won't typecheck yet! But that's okay. The other games weren't tested, and you should do better: create a file `server/tests/games/tictactoe.spec.ts` and write some tests for your `ticTacToeLogic`. This may help you think through what your `TicTacToeView` type actually needs to look like.
