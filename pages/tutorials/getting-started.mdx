...
- [Installing NodeJS](tutorials/getting-started#installing-nodejs)
- [Installing Visual Studio Code](tutorials/getting-started#installing-visual-studio-code-vscode)
- [Hello, World in TypeScript](tutorials/getting-started#getting-started-with-typescript)
...
**For this class, you will need Node.js version 23 (v23.11.0 was the latest version at time of writing, although any 23.x should work in theory).**

There are many ways that you can install Node.js: for instance, you can use a package manager like `snap` or `homebrew` to install it; you can download an installer directly from the Node.js website, and you can, of course, build it from source.

Due to the complexity of running different Node.js versions on the same machine, we _very strongly_ suggest using [nvm, the node version manager](https://github.com/nvm-sh/nvm). When language runtimes are in active development (like Node.js is), sometimes you end up needing to have multiple versions of Node.js installed, and different projects that you work on might require different versions of Node.js. These annoyances are quite rare, but when it happens that you need to have multiple versions of Node.js installed, it's super handy to have your system set up already so that installing multiple versions and switching between it is easy. You can use our instructions to set up nvm even if you
...
1. Download `nvm-setup.exe` from the most recent release of [nvm-windows](https://github.com/coreybutler/nvm-windows/releases) (at time of writing this document, version was 1.2.2).
   ...
   ![image](./getting-started/node/download-nvmw.png)
...
4. Upon completion, you will see the below window
   ...
   ![image](./getting-started/node/nvmw-finished.png)
...
6. Verify the installation, run the command `nvm version`
   - This should display the version of nvm installed, like this:
   
     ![image](./getting-started/node/nvmw-verification.png)
7. Install Node.js version 23 using the command `nvm install 23`.
8. To use this version of NodeJS, run the command `nvm use 23`.
...
This tutorial assumes you're using the Bash or Zsh shells (if you don't know what that means, you're probably using one of those). If you use the Fish shell, there are [fish instructions here](tutorials/getting-started/fish).
...
1. Run either
   ...
   or, if `wget` is installed, then run
...
3. Verify nvm is working by entering

   ```
   command -v nvm
   ```

   If your terminal prints out `nvm`, it should be working. If you see `nvm: command not found` or no feedback, open a new terminal and trying again or restart from step 1.

4. Install the required version of Node.js by typing `nvm install 23`.

5. To use this version of NodeJS, run the command `nvm use 23`.
...
- This should print the current version of nodeJS installed (23.x.x).

3. Run the command `npm -v`

- This should print the current version of npm installed (v10.x.x).
- ![image](./getting-started/node/verification.png)

4. If you find that some other version is being used, run the command `nvm use 23`. For Linux / Mac, you can change the default to 23 by running the command `nvm alias default 23`.

> Note: The `alias` command is not compatible with `nvm.fish`. Instead, you can
> add a default version with the command `exec nvm use 23` in your `~/.config/fish/config.fish` file.
...
8. Once the installation is complete, you will see the below screen and you are ready to use VSCode.
   - ![image](./getting-started/vsc/finish.JPG)
...
![image](./getting-started/vsc/settings.png)
...
- NodeJS
- VSCode (strongly recommended but not required)
...
4. Hover over the `message` — TypeScript will tell you that the `message` variable has been inferred to be a `string`.

   ![image](./getting-started/vsc/hover.png)

4. Open the terminal with ctrl + `~` or ctrl + '`' (ctrl-backtick)

   - Ensure that you are in the same directory as `hello-world.ts` (the `ls` command checks what ).
...
7. Run the command `npx ts-node hello-world.ts`.
   ...
   - This will give the result below.
     ...
     ![image](./getting-started/ts/result.png)
